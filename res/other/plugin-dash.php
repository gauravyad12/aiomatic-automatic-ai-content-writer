...
if ( !class_exists( 'CodeRevoDashboard' ) ) {
	class CodeRevoDashboard {
		...
		function show_dashboard_widget() {
		    ...
		    if ( is_wp_error($rss) ) {
				if ( is_admin() || current_user_can('access_aiomatic_menu') ) {
				   echo '<p>';
            ...
            wp_enqueue_script('coderevo-other-script', plugins_url('script.js', __FILE__), array('jquery'), AIOMATIC_MAJOR_VERSION);
            echo $str;
			$rss->__destruct();
			unset($rss);
		}
...
if ( !class_exists( 'CodeRevoNewsDashboard' ) ) {
	class CodeRevoNewsDashboard {
		...
		function show_dashboard_widget() {
            ...
            wp_enqueue_script('coderevo-other-script-news', plugins_url('scriptnews.js', __FILE__), array('jquery'), AIOMATIC_MAJOR_VERSION);
			echo $str;
			$rss->__destruct();
			unset($rss);
		}
