...
function aiomatic_add_processed_keyword($keyword) 
{
    ...
    if (!in_array($keyword, $processed_keywords)) 
    {
        ...
        update_option('aiomatic_processed_keywords', $processed_keywords, false);
    }
}
...
function addCharacterToVisibleText($html, $character) {
    $dom = new DOMDocument();
    $internalErrors = libxml_use_internal_errors(true);
...
function aiomatic_get_transients_by_regex($pattern, $users_per_page, &$trnsi_cnt) 
{
    ...
    $transients = $wpdb->get_col("
        SELECT option_name
        FROM $wpdb->options
        WHERE option_name REGEXP '^_transient_" . $wpdb->esc_like($pattern) . "'
    ");
...
function aiomatic_save_term_seo_description($term_id, $description, $taxonomy)
{
    ...
    if($seo_plugin_activated == '_yoast_wpseo_metadesc')
    {
        ...
        update_option('wpseo_taxonomy_meta', $yoast_options);
    }
    elseif($seo_plugin_activated == '_aioseo_description')
    {
...
function aiomatic_media_sideload_image( $file, $post_id = 0, $desc = null, $return_type = 'html', $file_name = '' ) {
	if ( ! empty( $file ) ) 
    {
        ...
        {
            return new WP_Error( 'File download error: ' . $file );
        }
		if ( is_wp_error( $file_array['tmp_name'] ) ) {
...
function replaceAIPostShortcodes($content, $post_link, $post_title, $blog_title, $post_excerpt, $post_content, $user_name, $featured_image, $post_cats, $post_tagz, $post_id, $img_attr = '', $old_title = '', $post_title_keywords = '', $custom_shortcodes = '', $global_prepend = '', $global_append = '')
{
    $matches = array();
    $i = 0;
    ...
    if (is_array($matches) && count($matches) && is_array($matches[0])) 
    {
        ...
        {
            if (isset($matches[0][$i])) $fullmatch = $matches[0][$i];
            if (isset($matches[1][$i])) $search_in = replaceAIPostShortcodes($matches[1][$i], $post_link, $post_title, $blog_title, $post_excerpt, $post_content, $user_name, $featured_image, $post_cats, $post_tagz, $post_id, $img_attr, $old_title, $post_title_keywords, $custom_shortcodes, $global_prepend, $global_append);
            if (isset($matches[2][$i])) $matchpattern = $matches[2][$i];
            if (isset($matches[3][$i])) $element = $matches[3][$i];
            if (isset($matches[4][$i])) $delimeter = $matches[4][$i];if (isset($matches[5][$i])) $counter = $matches[5][$i];
            if (isset($matchpattern)) {
    ...
    else
    {
        ...
        if(!is_array($custom_shortcodes))
        {
            ...
            {
                $custom_shortcodes = replaceAIPostShortcodes($custom_shortcodes, $post_link, $post_title, $blog_title, $post_excerpt, $post_content, $user_name, $featured_image, $post_cats, $post_tagz, $post_id, $img_attr, $old_title, $post_title_keywords, '', $global_prepend, $global_append);
            }
            ...
            if(isset($name_part[1]) && !empty(trim($name_part[1])))
            {
                ...
                if(strstr($content, '%%' . $shortname . '%%'))
                {
                    ...
                    if(isset($ai_part[1]) && !empty(trim($ai_part[1])))
                    {
                        ...
                        if(!empty($ai_command))
                        {
                            $ai_command = replaceAIPostShortcodes($ai_command, $post_link, $post_title, $blog_title, $post_excerpt, $post_content, $user_name, $featured_image, $post_cats, $post_tagz, $post_id, $img_attr, $old_title, $post_title_keywords, $custom_shortcodes, $global_prepend, $global_append);
                        }
                        $presence_penalty = 0;
                        $frequency_penalty = 0;
                        ...
                        $generated_text = aiomatic_generate_text($token, $aimodel, $ai_command, $available_tokens, $temperature, $top_p, $presence_penalty, $frequency_penalty, false, 'customShortcode', 0, $finish_reason, $aierror, true, false, false, '', '', 'user', '', $thread_id, '', 'disabled', '', false);
    ...
    if (isset($aiomatic_Main_Settings['use_image_ai']) && $aiomatic_Main_Settings['use_image_ai'] === 'on') 
    {
        ...
        else
        {
            ...
            else
            {
                ...
                $aierror = '';
                $finish_reason = '';
                $generated_text = aiomatic_generate_text($token, $model, $prompt, $available_tokens, 1, 1, 0, 0, false, 'AIImageSelector', 0, $finish_reason, $aierror, false, false, false, '', '', 'user', $img_assistant_id, $thread_id, '', 'disabled', '', false);
                if($generated_text === false)
                {
                    if (isset($aiomatic_Main_Settings['enable_detailed_logging'])) {
...
function aiomoatic_get_video($new_post_title, $chance = '')
{
    ...
    if (isset($aiomatic_Main_Settings['improve_yt_kw']) && $aiomatic_Main_Settings['improve_yt_kw'] === 'on') 
    {
        ...
        else
        {
            ...
            else
            {
                ...
                $aierror = '';
                $finish_reason = '';
                $generated_text = aiomatic_generate_text($token, $model, $prompt, $available_tokens, 1, 1, 0, 0, false, 'YouTubeKeywordWriter', 0, $finish_reason, $aierror, false, false, false, '', '', 'user', $yt_assistant_id, $thread_id, '', 'disabled', '', false);
                if($generated_text === false)
                {
...
function aiomatic_is_vision_model($model, $assistant_id)
{
    if(!empty($assistant_id))
    {
        ...
        if($local_assist !== false)
        {
            ...
            {
                if(in_array($assist_model, AIOMATIC_MODELS_VISION))
                {
                    return true;
                }
                return false;
    ...
    if(in_array($model, AIOMATIC_MODELS_VISION))
    {
        return true;
    }
    return false;
...
function aiomatic_get_ollama_models($no_cache = false)
{
    ...
    if(stristr($cmdResult, '"models"') !== false)
    {
        ...
        update_option('aiomatic_ollama_models', $rules);
    }
    else
    {
...
{
    if(aiomatic_is_chatgpt_turbo_model($model) || aiomatic_is_chatgpt35_16k_context_model($model) || aiomatic_is_chatgpt_o_model($model))
    {
        return true;
    }
    return false;
